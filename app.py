...
app = Flask(__name__)
app.secret_key = "jfda;fjieurqeri3048fdk"
...
def insert_roti():
    ...
    nama_roti = request.form.get('nama_roti')
    id_kat = request.form.get('id_kategori')
    deskripsi = request.form.get('deskripsi')
    harga = request.form.get('harga')
    stok = request.form.get('stok')
    DaftarRoti.create(nama_roti, id_kat, deskripsi, harga, stok)
    return redirect(url_for('index'))
...
def update_roti(id_roti):
    ...
    harga = request.form.get('harga')
    stok = request.form.get('stok')
    DaftarRoti.update(id_roti, nama_roti, id_kat, deskripsi, harga, stok)
    return redirect(url_for('index'))
...
def register():
    ...
    if not admin_only():
        return "Hanya admin yang dapat menambah user baru."
